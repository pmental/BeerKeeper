...
- **0.0.54** — Beer styles moved from a hand-edited text file into the database, with a new "Beer Styles" panel on the admin page (add/rename/delete) - matching how breweries and beers are already managed there. An existing install's hand-edited `beer_styles.txt` is migrated in automatically on first boot after upgrading, preserving any customizations; a fresh install seeds from the same bundled default list as before. This also simplifies backup/restore, which no longer needs to special-case a second file alongside the database - styles are just part of it now. Trimmed two sentences from the OIDC section of the README.

- **0.0.53** — Updated every pinned dependency to its current latest version: fastapi 0.115.0→0.141.1, uvicorn 0.30.6→0.52.4, sqlalchemy 2.0.35→2.0.52, pydantic 2.9.2→2.13.5, python-jose 3.3.0→3.5.0, bcrypt 4.2.0→5.0.0, python-multipart 0.0.9→0.0.32, email-validator 2.2.0→2.3.0, authlib 1.3.2→1.8.0, httpx 0.27.2→0.28.1 (itsdangerous was already current). Found and fixed a real compatibility gap this surfaced: bcrypt 5.0 now raises an error on passwords over 72 bytes instead of silently truncating them, but the app's own password fields allowed up to 200 characters - a password between 73-200 (ASCII) characters would have crashed instead of registering. Added proper validation that checks the actual UTF-8 byte length (not character count, so it's also correct for passwords using accents, emoji, or non-Latin scripts) and gives a clear error instead of a server error. Verified the full password lifecycle (register, login, change password) under the new bcrypt, confirmed OIDC login still works under the upgraded authlib/httpx, and reran the complete regression suite - all clean.
